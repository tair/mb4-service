...
export class PartitionProjectPublisher extends BaseModelDuplicator {
  ...
  async generateSQLStatementForTable(tableName) {
    switch (tableName) {
      ...
      case 'matrix_character_order':
        matrixIds = join(', ', this.getAllIdsInTable('matrices'))
        return (
          `SELECT matrix_character_order.*
						FROM matrix_character_order
						INNER JOIN characters USING(character_id)
						INNER JOIN characters_x_partitions USING(character_id)
						WHERE
							matrix_character_order.matrix_id IN ` +
          matrixIds +
          ` AND
							characters_x_partitions.partition_id = ` +
          this.partitionId +
          ` AND
      ...
      case 'character_rule_actions':
        ...
        return (
          `SELECT character_rule_actions.*
						...
						WHERE
							cxp_mother.partition_id = + ` +
          this.partitionId +
          ` AND
							cxp_daugther.partition_id = ` +
          this.partitionId +
          ` AND
							characters.project_id = ?`
        )
      ...
      case 'specimens':
        ...
        return (
          `SELECT specimens.*
						...
						WHERE media_files.media_id IN ` +
          mediaIds +
          ` AND specimens.project_id = ?
						GROUP BY specimens.specimen_id`
        )
      case 'specimens_x_bibliographic_references':
        mediaIds = join(', ', this.getAllIdsInTable('media_files'))
        return (
          `SELECT specimens_x_bibliographic_references.*
      ...
      case 'media_views':
        ...
        return (
          `SELECT media_views.*
						...
						WHERE media_files.media_id IN ` +
          mediaIds +
          ` AND media_views.project_id = ?
						GROUP BY media_views.view_id`
        )
      case 'taxa_x_specimens':
        mediaIds = join(', ', this.getAllIdsInTable('media_files'))
        return (
          `SELECT taxa_x_specimens.*
						FROM taxa_x_specimens
						INNER JOIN taxa_x_partitions USING(taxon_id)
						INNER JOIN specimens USING(specimen_id)
						INNER JOIN media_files USING(specimen_id)
						WHERE
							taxa_x_partitions.partition_id = ` +
          this.partitionId +
          ` AND
							media_files.media_id IN ` +
          mediaIds +
          ` AND
      ...
      case 'media_labels':
        ...
        return (
          `SELECT media_labels.*
						...
						WHERE media_labels.media_id IN ` +
          mediaIds +
          ` AND media_files.project_id = ?
      ...
      case 'project_documents':
        ...
        return (
          `SELECT project_documents.*
						...
						WHERE media_files_x_documents.media_id IN ` +
          mediaIds +
          ` AND project_documents.project_id = ?
						GROUP BY project_documents.document_id`
        )
      case 'project_document_folders':
        mediaIds = join(', ', this.getAllIdsInTable('media_files'))
        return (
          `SELECT project_document_folders.*
  ...
  static async clonePartitionAsNewProject(options = null) {
    ...
    let modelDuplicator = new PartitionProjectPublisher(
      'projects',
      projectId,
      partitionId
    )
    ...
    if (!clonedProjectId) {
      ...
      console.log(
        '[ERROR] Error duplicating project P{projectId} (' +
          console.timeEnd() +
          's)\n.'
      )
      return null
    ...
    clonedProject.set(
      'name',
      project.get('name') + ' (from P'.project.get('project_id') + ')'
    )
    ...
    if ((exemplarMediaId = clonedProject.get('exemplar_media_id'))) {
      exemplarMediaId = modelDuplicator.getDuplicateRecordId(
        'media_files',
        exemplarMediaId,
        false
      )
    ...
    console.log(
      '[INFO] Copied P{projectId} to P{clonedProjectId}, done in ' +
        console.timeEnd() +
        's\n'
    )
    transaction.commit()
