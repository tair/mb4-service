...
export class BaseModelScanner {
  ...
  setNumberedTables(numberedTables) {
    this.numberedTables = numberedTables
  }
  ...
  getTopologicalDependentTables() {
    ...
    // rest of the tables.
    const numbered = Array.from(this.numberedTables.keys())
    const diff = dependentTables.filter(
      (table) => !this.numberedTables.has(table)
    )
