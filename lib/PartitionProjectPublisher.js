...
export class PartitionProjectPublisher {
	...
	//TODO: Figure out PHP to JS refactor for Timer, SearchIndexer, and AUTH_CURRENT_USER
	async static clonePartitionAsNewProject(options = null) {
		...
		// Set the global current user if it's not available. This ensures that logs are written currently.
		let userId = options['user_id'];
		global $AUTH_CURRENT_USER_ID;
		if (!$AUTH_CURRENT_USER_ID) {
			$AUTH_CURRENT_USER_ID = userId;
		}
		...
		if (!clonedProjectId) {
			...
			console.log("[ERROR] Error duplicating project P{projectId} (" + parseFloat($o_timer.getTime()) + "s)\n.");
			return null;
		}
		...
		clonedProject.set('created_on', time());
		clonedProject.set('last_accessed_on', time());
		...
		projectsXUsers.set('created_on', time());
		...
		console.log("[INFO] Copied P{projectId} to P{clonedProjectId}, done in " + parseFloat($o_timer.getTime()) + "s\n");

		transaction.commit();

		//exec("php ". __CA_BASE_DIR__ ."/support/utils/mb_overview_queries.php.php {clonedProjectId}");

		// Reindex the newly added rows because they were not added to the search index.
		$o_timer.startTimer();
		$o_indexer = new SearchIndexer(db);
		transaction.start();
		$o_indexer.reindexRows(modelDuplicator.getDuplicatedRecords());
		transaction.commit();

		console.log("[INFO] Completed indexing and stats generation of P{clonedProjectId}, done in " + parseFloat($o_timer.getTime()) + "s\n");
